...
# large_RNA_pb 
# atac = fread("~/Documents/Research/t21-proj/out/full/DE_pb_leiden_names/Liver.HSCs_MPPs.sample.txt",data.table = F,stringsAsFactors = F)
# atac = fread("/oak/stanford/groups/smontgom/amarder/t21_multiome/output/differential_analysis/Liver.HSCs_MPPs.sample.txt",data.table = F,stringsAsFactors = F)
...
atac = fread("/oak/stanford/groups/smontgom/amarder/t21_multiome/output/differential_analysis/HSC_v_CyclingHSC.txt",data.table = F,stringsAsFactors = F)
...
# for (k in unique(data_snv$fetus)) {
  ...
  # atac.df = atac[,c("chromosome_name","start_position","end_position")]
  # colnames(atac.df) = c("chrom","start","end")
  ...
  # atac.df$start = atac.df$start - 100000
  # atac.df$end = atac.df$start + 100000
